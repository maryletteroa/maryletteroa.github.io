...
- `invalidate_hard_deletes = True` means that if the record is deleted, dbt will replace the `dbt_valid_to` date to the snapshot date; if equal to `False`, it will not capture the delete
