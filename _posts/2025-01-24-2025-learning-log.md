...
- `invalidate_hard_deletes = True` means that if the record is deleted, dbt will replace the `dbt_valid_to` date to the snapshot date rather; if equal to `False`, it will not capture the delete
...
- There are two kinds of tests in the dbt
  ...
  - generic
    - built_ind tests from the core installation of dbt
      - unique
      - not_null
      - accepted_values
