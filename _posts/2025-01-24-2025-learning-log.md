...
## 7: 2025-02-01 (Sat)
Today isn't as technical as the rest of the lectures.  I've covered more features of the Power User for dbt core extension powered with AI, and tips on introducing dbt to the company.

The course is wrapping up as well, and I only have one full section left about certifications. I will clean this log up a bit and perhaps put dbt the notes up into a separate post. 
...
### Introducing dbt to company
As for introducing dbt to the company, I find these tips from the interview interesting:
- Approach it as an experimentation rather than a business case. Experimentation means you're going to try if it's a good fit. This also would put less pressure on the team and give them time to learn the tool and also find out if they like it or not -- rather than introducing it as a business case wherein it's presented as *the* solution.
...
- dbt abstracts the technicalities like parallelization, dependency of models, optimization of execution, so developers can focus is on the logic and model building
