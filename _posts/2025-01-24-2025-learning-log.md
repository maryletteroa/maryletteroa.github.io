...
## Documentation
- Should be kept close close to the actual code as possible
- Defined in two ways
...
- run `dbt docs generate` to generate docs at `target` folder 
  ...
  - html path `dbtlearn/target/index.html`
...
- crete a markdown file e.g. in the `models` directory `dbtlearn/models/docs.md`
...
## Analyses
- run a query using dbt but not necessarily create a model
- version control, and execute against the datawarehouse
...
```
- run `dbt compile`
- parsed sql in `dbtlearn/target/compiled/dbtlearn/analyses/full_moon_no_sleep.sql` which can be run in the data warehouse
...
e.g. add a REPORTER role in Snowflake
...
- create a yaml file `dbtlearn/models/dashboards.yml`
...
- also pops up in the lineage graph
