...
I've covered two topics: `dbt-great-expectations` and logging
...
- e.g. regex for price $90.00
	- `$` and `.` need to escaped for regex, then escaped for YAML, then need to be escaped twice so the slashes actually render in Snowflake; these happens when there are many interconnected technologies
...
### Logging
- can log messages to dbt's standard log file, or the terminal, or disable log a message temporarily
